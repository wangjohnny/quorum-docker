...
RUN apt-get update && \
    apt-get install -y wget git build-essential libdb-dev libleveldb-dev libsodium-dev zlib1g-dev libtinfo-dev solc sysvbanner wrk
...
RUN cp $CONSTELLATION_REL/constellation-node /usr/local/bin && \
